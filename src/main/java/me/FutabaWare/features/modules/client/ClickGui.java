...
public class ClickGui
    ...
    public void onSettingChange(ClientEvent event) {
        if (event.getStage() == 2 && event.getSetting().getFeature().equals(this)) {
            if (event.getSetting().equals(this.prefix)) {
                FutabaWare.commandManager.setPrefix(this.prefix.getPlannedValue());
                Command.sendMessage("Prefix set to " + ChatFormatting.DARK_GRAY + FutabaWare.commandManager.getPrefix());
            }
            FutabaWare.colorManager.setColor(this.red.getPlannedValue(), this.green.getPlannedValue(), this.blue.getPlannedValue(), this.hoverAlpha.getPlannedValue());
        }
    }

    @Override
    public void onEnable() {
        Util.mc.displayGuiScreen(FutabaWareGui.getClickGui());
    }

    @Override
    public void onLoad() {
        FutabaWare.colorManager.setColor(this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.hoverAlpha.getValue());
        FutabaWare.commandManager.setPrefix(this.prefix.getValue());
    }

    @Override
    public void onTick() {
        if (!(ClickGui.mc.currentScreen instanceof FutabaWareGui)) {
            this.disable();
        }
    }
    ...
    public void drawImageLogo() {
        ...
        FutabaWareGui.drawCompleteImage(ClickGui.getInstance().imageX.getValue(), ClickGui.getInstance().imageY.getValue(), ClickGui.getInstance().imageWidth.getValue(), ClickGui.getInstance().imageHeight.getValue());
