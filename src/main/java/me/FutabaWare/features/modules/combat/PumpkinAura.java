...
import me.FutabaWare.event.events.TotemPopEvent;
...
public class PumpkinAura extends Module {
    ...
    public void placePumpkin() {
        List<EntityPlayer> players = mc.world.playerEntities.stream().filter(
                player -> !player.equals(mc.player) && player.getDistance(mc.player) <= 12f && !(player.isDead || player.getHealth() <= 0) && !FutabaWare.friendManager.isFriend(player.getName())
