...
import me.FutabaWare.event.events.MoveEvent;
...
import me.FutabaWare.mixin.mixins.accessors.IEntityLivingBase;
...
public class EntityUtil
    ...
    private static boolean isValid(Entity entity, boolean bl, boolean bl2, boolean bl3, boolean bl4, boolean bl5, double d) {
        ...
        if (entity instanceof EntityLivingBase && entity != EntityUtil.mc.player && entity.getDistanceSq((Entity)EntityUtil.mc.player) <= d * d) {
            if (entity instanceof EntityPlayer && bl) {
                if (!bl3) {
                    return !FutabaWare.friendManager.isFriend((EntityPlayer)entity);
                }
                return true;
            }
    ...
    public static boolean isCrystalAtFeet(final EntityEnderCrystal crystal, final double range) {
        for (final EntityPlayer player : mc.world.playerEntities) {
            ...
            if (FutabaWare.friendManager.isFriend(player)) {
                continue;
            }
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || EntityUtil.isDead(entity) || entity.equals(EntityUtil.mc.player) || entity instanceof EntityPlayer && FutabaWare.friendManager.isFriend(entity.getName()) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(Entity entity, int red, int green, int blue, int alpha, boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer && colorFriends && FutabaWare.friendManager.isFriend((EntityPlayer) entity)) {
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            output.put(healthSB.toString() + " " + (FutabaWare.friendManager.isFriend(player) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + " " + distanceSB.toString() + " \u00c2\u00a7f0", (int) EntityUtil.mc.player.getDistance(player));
