...
import me.FutabaWare.features.setting.EnumConverter;
...
public class ConfigManager implements Util {
    ...
    public String config = "FutabaWare/config/";
    ...
    public static void setValueFromJson(Feature feature, Setting setting, JsonElement element) {
        ...
        FutabaWare.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
    }
    ...
    private static void loadFile(JsonObject input, Feature feature) {
        for (Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    FutabaWare.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                continue;
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("FutabaWare").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("FutabaWare/" + name + "/"))) {
            this.config = "FutabaWare/" + name + "/";
        } else {
            this.config = "FutabaWare/config/";
        }
        FutabaWare.friendManager.onLoad();
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("FutabaWare").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("FutabaWare/" + name + "/"));
    }
    ...
    public void saveConfig(String name) {
        ...
        if (!path.exists())
            path.mkdir();
        FutabaWare.friendManager.saveFriends();
        for (Feature feature : this.features) {
            try {
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("FutabaWare/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("FutabaWare", ""));
                writer.close();
            } else {
    ...
    public void init() {
        this.features.addAll(FutabaWare.moduleManager.modules);
        this.features.add(FutabaWare.friendManager);
        String name = loadCurrentConfig();
        loadConfig(name);
        FutabaWare.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(Path path, Feature feature) throws IOException {
        ...
        try {
            ...
            FutabaWare.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
